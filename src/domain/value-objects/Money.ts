...
export class Money {
  ...
  constructor(amount: number, currency: string = 'USD') {
    ...
    if (!Number.isInteger(amount)) {
      throw new ValidationError('Amount must be an integer (representing cents)');
    }
  ...
  percentage(percentage: number): Money {
    ...
    return new Money(Math.round((this._amount * percentage) / 100), this._currency);
  }
  ...
  private ensureSameCurrency(other: Money): void {
    if (this._currency !== other._currency) {
      throw new ValidationError('Cannot perform operations on different currencies');
    }
  }
  ...
  static min(...monies: Money[]): Money {
    ...
    if (!monies.every(m => m.currency === firstCurrency)) {
      throw new ValidationError('All money objects must have the same currency');
    }

    return monies.reduce((min, current) => (current.lessThan(min) ? current : min));
  }
  ...
  static max(...monies: Money[]): Money {
    ...
    return monies.reduce((max, current) => (current.greaterThan(max) ? current : max));
  }
  ...
  static sum(...monies: Money[]): Money {
    ...
    return monies.reduce((sum, current) => sum.add(current), Money.zero(firstCurrency));
  }
}
