...
class NSFWDetectionService {
    ...
    try {
      ...
      if (this.config.enableMultiModelAnalysis) {
        for (const modelId of this.config.modelsToUse) {
          const modelConfig = NSFW_DETECTION_MODELS.find(
            (m) => m.id === modelId,
          );
          if (modelConfig) {
  ...
  private buildSafetyPrompt(imageBase64: string, modelConfig: any): string {
    const sensitivityPrompts = {
      ...
      medium:
        "Use moderate detection - flag explicit and highly suggestive content.",
...
Focus on maximum accuracy and comprehensive safety assessment.`;
  ...
  private calculateConsensus(
    ...
    const avgConfidence =
      modelResults.reduce((sum, r) => sum + r.confidence, 0) /
      modelResults.length;
  ...
  private calculateConsensusScore(modelResults: ModelAnalysisResult[]): number {
    ...
    // Calculate weighted consensus
    const nsfwWeight =
      nsfwResults.reduce((sum, r) => sum + r.confidence, 0) /
      nsfwResults.length;
    const safeWeight =
      safeResults.reduce((sum, r) => sum + r.confidence, 0) /
      safeResults.length;
  ...
  private calculateDetailedAnalysis(modelResults: ModelAnalysisResult[]): any {
    ...
    return {
      explicitContent: this.averageBoolean(
        analyses.map((a) => a.explicitContent),
      ),
      suggestiveContent: this.averageBoolean(
        analyses.map((a) => a.suggestiveContent),
      ),
      ...
      deepfakeDetected: this.averageBoolean(
        analyses.map((a) => a.deepfakeDetected),
      ),
      manipulatedContent: this.averageBoolean(
        analyses.map((a) => a.manipulatedContent),
      ),
    };
  }
    ...
    if (isNsfw) {
      ...
      if (riskLevel === "critical") {
        ...
        recommendations.push(
          "Consider reporting to authorities if illegal content",
        );
      }
    } else {
