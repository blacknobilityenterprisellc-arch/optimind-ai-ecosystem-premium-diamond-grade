...
class AIEnhancementEngine {
  ...
  private async detectFaces(base64Image: string): Promise<number> {
    try {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert face detection specialist. Count faces accurately.",
          },
          {
            role: "user",
            content: prompt,
      ...
      const content =
        response.choices[0]?.message?.content || '{"faceCount": 0}';
      const parsed = JSON.parse(content);
  ...
  private async analyzeAesthetics(base64Image: string): Promise<number> {
    try {
      ...
      const content =
        response.choices[0]?.message?.content || '{"aestheticScore": 0.5}';
      const parsed = JSON.parse(content);
  ...
  private async generateEnhancedTags(
    analysis: AIAnalysisResult,
  ): Promise<string[]> {
    try {
      const prompt = `
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert tagging specialist. Generate comprehensive, relevant tags.",
          },
          {
            role: "user",
            content: prompt,
  ...
  private generateCategories(analysis: AIAnalysisResult): string[] {
    ...
    if (
      analysis.objects.includes("car") ||
      analysis.objects.includes("vehicle")
    )
      categories.push("Transportation");
    if (
      analysis.objects.includes("building") ||
      analysis.objects.includes("house")
    )
    ...
    if (
      analysis.emotions.includes("sad") ||
      analysis.emotions.includes("melancholic")
    )
  ...
  async detectPrivacyConcerns(analysis: AIAnalysisResult): Promise<string[]> {
    ...
    if (
      analysis.emotions.includes("sad") ||
      analysis.emotions.includes("angry")
    ) {
