...
class DomainExtractionService {
  ...
  async extractData(request: ExtractionRequest): Promise<ExtractionResult> {
    ...
    try {
      ...
      if (!agentConfig) {
        throw new Error(
          `Agent profile not found: ${domainConfig.agentProfile}`,
        );
      }
      ...
      const validatedResult = await this.validateAndEnhanceResults(
        extractionResult,
        request,
      );
  ...
  private validateExtractionRequest(
    ...
    // Validate domain
    if (
      !["healthcare", "legal", "education", "customer-service"].includes(
        request.domain,
      )
    ) {
    ...
    switch (request.domain) {
      case "healthcare":
        if (
          ...
          errors.push(
            "Invalid jurisdiction specified for healthcare extraction",
          );
          isValid = false;
        }
        break;
  ...
  private async generateContextPrompt(
    ...
    const response =
      await premiumContextEngineeringService.generateContextPrompt(
        contextRequest,
      );

    if (!response.success || !response.prompt) {
      throw new Error(
        `Failed to generate context prompt: ${response.errors.join(", ")}`,
      );
    }
  ...
  private buildAgentProfile(
    ...
    let profile =
      profiles[request.domain] ||
      "You are a domain expert data extraction specialist.";
  ...
  private buildOutputFormat(request: ExtractionRequest): string {
    ...
    return (
      formats[request.domain] || '{"extractedData": [], "confidence": number}'
    );
  }
    ...
    for (const toolId of domainConfig.tools) {
      try {
        const toolRequest: MCPToolRequest = {
          toolId,
          parameters: this.prepareToolParameters(
            toolId,
            request,
            contextPrompt,
          ),
    ...
    switch (toolId) {
      ...
      case "legal-clause-extractor":
        return {
          ...
          clauseTypes: request.options?.extractTypes || [
            "clauses",
            "parties",
            "dates",
          ],
      ...
      case "data-analysis":
        return {
          ...baseParams,
          dataset: Array.isArray(request.inputData)
            ? request.inputData
            : [request.inputData],
  ...
  private deduplicateArray(array: any[]): any[] {
    ...
    return array.filter((item) => {
      const key =
        item.id || item.term || item.name || item.type || JSON.stringify(item);
      if (seen.has(key)) {
        return false;
      }
      seen.add(key);
  ...
  private async validateAndEnhanceResults(
    result: any,
    request: ExtractionRequest,
  ): Promise<any> {
    ...
    switch (request.domain) {
      case "healthcare":
        enhancedResult.data = await this.validateHealthcareData(
          enhancedResult.data,
          request,
        );
        break;
      case "legal":
        enhancedResult.data = await this.validateLegalData(
          enhancedResult.data,
          request,
        );
        break;
      case "education":
        enhancedResult.data = await this.validateEducationData(
          enhancedResult.data,
          request,
        );
        break;
      case "customer-service":
        enhancedResult.data = await this.validateCustomerServiceData(
          enhancedResult.data,
          request,
        );
        break;
    ...
    enhancedResult.confidence = this.calculateOverallConfidence(
      enhancedResult.data,
    );
  ...
  private async validateLegalData(
    data: any,
    request: ExtractionRequest,
  ): Promise<any> {
  ...
  private async validateEducationData(
    data: any,
    request: ExtractionRequest,
  ): Promise<any> {
    ...
    if (data.learningObjectives) {
      validated.learningObjectives = data.learningObjectives.map(
        (obj: any) => ({
          ...obj,
          measurable: this.validateObjectiveMeasurability(obj.description),
          ageAppropriate: this.validateAgeAppropriateness(
            obj,
            request.options?.educationLevel,
          ),
        }),
      );
    }
  ...
  private async validateCustomerServiceData(
    data: any,
    request: ExtractionRequest,
  ): Promise<any> {
    ...
    if (data.actionItems) {
      validated.actionItems = data.actionItems.map((item: any) => ({
        ...
        priorityValidated: this.validatePriority(
          item.priority,
  ...
  private async performComplianceChecks(
    ...
    switch (request.domain) {
      case "healthcare":
        compliance.hipaa = await this.checkHIPAACompliance(
          result.data,
          request,
        );
  ...
  private updatePerformanceMetrics(result: ExtractionResult): void {
    ...
    const updated = {
      ...
      successfulExtractions:
        existing.successfulExtractions + (result.success ? 1 : 0),
      averageConfidence:
        (existing.averageConfidence * existing.totalExtractions +
          result.confidence) /
      ...
      averageProcessingTime:
        ...
        (existing.totalExtractions + 1),
      errorRate:
        (existing.errorRate * existing.totalExtractions +
          (result.success ? 0 : 1)) /
        (existing.totalExtractions + 1),
    };
