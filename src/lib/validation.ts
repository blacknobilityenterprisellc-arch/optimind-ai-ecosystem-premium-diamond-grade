...
export class ValidationRules {
  ...
  static passwordStrength(message: string = 'Password must be at least 8 characters with uppercase, lowercase, number, and special character'): ValidationRule {
    return {
  ...
  // Username validation
  static username(message: string = 'Username must be 3-20 characters with letters, numbers, and underscores only'): ValidationRule {
    return {
      name: 'username',
      validate: (value) => {
        if (typeof value !== 'string') return false;
  ...
  static file(allowedTypes: string[], maxSize: number, message?: string): ValidationRule {
    return {
      ...
      message: message || `File must be one of: ${allowedTypes.join(', ')} and less than ${maxSize / 1024 / 1024}MB`
    };
  }
}
...
export class Validator {
  ...
  async validate(data: Record<string, any>, options?: {
    strict?: boolean;
  ...
  // Add warning
  addWarning(field: string, message: string, value: any, severity: 'low' | 'medium' | 'high' = 'medium'): void {
    this.warnings.push({
      field,
      message,
      value,
...
export const Schemas = {
  ...
  // User login schema
  userLogin: () => new SchemaBuilder()
    .requiredString('email')
    .requiredString('password')
    .build(),

  // Content creation schema
  contentCreation: () => new SchemaBuilder()
    .requiredString('title')
    .requiredString('content')
    .enum('type', ['blog', 'article', 'product-description', 'social-media', 'email', 'documentation'])
    .enum('status', ['draft', 'published', 'archived'], false)
    .array('tags', false)
    .build(),

  // AI request schema
  aiRequest: () => new SchemaBuilder()
