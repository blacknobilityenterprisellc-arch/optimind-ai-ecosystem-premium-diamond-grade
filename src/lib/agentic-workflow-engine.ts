...
class AgenticWorkflowEngine {
      ...
      },
      {
        ...
        tasks: [
          ...
          },
          {
            ...
            complexity: 'expert',
            input: { clauses: [] },
            expectedOutput: { compliant: boolean, violations: [], recommendations: [] }
          },
          {
            id: 'risk-assessment',
      ...
      },
      {
        ...
        tasks: [
          ...
          },
          {
            ...
            complexity: 'complex',
            input: { validatedData: [] },
            expectedOutput: { terminology: [], conditions: [], medications: [] }
          },
          {
            id: 'hipaa-compliance',
  ...
  // Workflow Execution
  async executeWorkflow(workflowId: string, inputData: any, options?: {
    reasoningMode?: 'thinking' | 'non-thinking' | 'hybrid';
    forceModel?: string;
    budget?: number;
  ...
  private determineReasoningApproach(task: AgenticTask, workflowMode: string): {
  ...
  private async executeWithThinkingMode(task: AgenticTask, context: any, config: ThinkingModeConfig): Promise<any> {
    ...
    execution.metadata.reasoningDepth = Math.max(execution.metadata.reasoningDepth, reasoningSteps.length);

    return finalResult;
  }

  private async executeWithNonThinkingMode(task: AgenticTask, context: any, config: NonThinkingModeConfig): Promise<any> {
  ...
  private async executeWithHybridMode(task: AgenticTask, context: any, config: HybridModeConfig): Promise<any> {
    const complexityScore = this.getComplexityScore(task.complexity);
    const { execution } = context;

    // Start with non-thinking mode for quick assessment
    const quickResult = await this.executeWithNonThinkingMode(task, context, config.nonThinkingConfig);
    ...
    if (config.autoSwitch && 
      ...
      const detailedResult = await this.executeWithThinkingMode(task, context, config.thinkingConfig);
  ...
  private async retryWithRefinedApproach(task: AgenticTask, context: any, validationResult: any, reasoningSteps: ReasoningStep[]): Promise<any> {
  ...
  private synthesizeResults(task: AgenticTask, aiResult: any, toolResults: any[], reasoningSteps: ReasoningStep[]): any {
  ...
  private estimateOpenRouterCost(response: OpenRouterAnalysisResponse): number {
    ...
    return response.usage ? 
      ...
      (response.usage.completionTokens / 1000) * 0.01 : 0.01;
  }
  ...
  getReasoningHistory(executionId: string, taskId?: string): ReasoningStep[] | Map<string, ReasoningStep[]> {
    if (taskId) {
  ...
  updatePerformanceMetrics(workflowId: string, execution: AgenticExecution): void {
    ...
    const updated = {
      ...
      successfulExecutions: existing.successfulExecutions + (execution.status === 'completed' ? 1 : 0),
      averageExecutionTime: (existing.averageExecutionTime * existing.totalExecutions + executionTime) / (existing.totalExecutions + 1),
      averageCost: (existing.averageCost * existing.totalExecutions + execution.metadata.totalCost) / (existing.totalExecutions + 1),
      averageTokensUsed: (existing.averageTokensUsed * existing.totalExecutions + execution.metadata.tokensUsed) / (existing.totalExecutions + 1)
    };
