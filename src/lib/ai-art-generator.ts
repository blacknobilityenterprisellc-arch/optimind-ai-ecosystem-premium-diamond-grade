...
class AIArtGeneratorService {
  private styles: ArtStyle[] = [
    ...
    },
    {
      ...
      name: "Artistic",
      description:
        "Painterly and artistic styles including oil painting, watercolor",
  ...
  async generateArt(
    ...
    try {
      ...
      const style =
        this.styles.find((s) => s.id === options.style) || this.styles[0];
      const model =
        this.models.find((m) => m.id === options.model) || this.models[0];
...
export function useAIArtGenerator() {
    ...
    async (
      ...
      try {
        ...
        const totalCredits = results.reduce(
          (sum, result) => sum + result.creditsUsed,
          0,
        );
  ...
  const upscaleImage = useCallback(
    async (imageUrl: string, targetWidth?: number, targetHeight?: number) => {
      try {
        setIsGenerating(true);
        setError(null);
        ...
        const errorMessage =
          err instanceof Error ? err.message : "Upscaling failed";
        setError(errorMessage);
        throw err;
      } finally {
        setIsGenerating(false);
