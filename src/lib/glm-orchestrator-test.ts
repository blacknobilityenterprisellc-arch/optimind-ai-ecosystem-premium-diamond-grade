...
import { glmOrchestrator, GLMOrchestrator, SystemHealthStatus, OrchestratedResult } from './glm-orchestrator';
...
export class GLMOrchestratorTest {
  ...
  private async testMultiAgentCoordination(): Promise<any> {
    ...
    const coordinationOperation = {
      ...
      agentRequirements: ['glm-4.5-flagship', 'glm-4.5-auto-think', 'glm-4.5v', 'glm-4.5-full-stack'],
  ...
  private generateTestReport(data: any): any {
    ...
    const report = {
      ...
      summary: {
        ...
        successfulOperations: data.operationResults.filter((r: OrchestratedResult) => r.success).length,
        averageProcessingTime: data.operationResults.reduce((sum: number, r: OrchestratedResult) => sum + r.processingTime, 0) / data.operationResults.length,
        averageConfidence: data.operationResults.reduce((sum: number, r: OrchestratedResult) => sum + r.confidence, 0) / data.operationResults.length
      },
      healthAnalysis: {
    ...
    console.log(`  - Success Rate: ${((report.summary.successfulOperations / report.summary.totalOperations) * 100).toFixed(1)}%`);
