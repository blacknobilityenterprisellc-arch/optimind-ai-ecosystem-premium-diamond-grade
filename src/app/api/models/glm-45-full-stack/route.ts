...
export async function POST() {
  try {
    ...
    if (messages && Array.isArray(messages) && messages.length > 0) {
      ...
      const result = await zaiApiService.generateCompletion(prompt, 'glm-45-full-stack', {
        customPrompt,
      });
    ...
    return NextResponse.json({ error: 'GLM-4.5 Full Stack analysis failed' }, { status: 500 });
  }
}
