...
async function getOverviewAnalytics() {
  ...
  const recentDataPoints = sensors.reduce((sum, sensor) => {
    const recent = sensor.data.filter(
      d => new Date(d.timestamp).getTime() > Date.now() - 24 * 60 * 60 * 1000
    );
  ...
  const averageUptime =
    sensors.reduce((sum, sensor) => sum + sensor.health.uptime, 0) / sensors.length || 0;
  const averageErrorRate =
    sensors.reduce((sum, sensor) => sum + sensor.health.errorRate, 0) / sensors.length || 0;

  // Device type distribution
  const deviceTypeDistribution = devices.reduce(
    (acc, device) => {
  ...
  const sensorCategoryDistribution = sensors.reduce(
    (acc, sensor) => {
  ...
  return NextResponse.json({
    ...
    data: {
      ...
      sensors: {
        ...
        healthyPercentage:
          totalSensors > 0 ? ((healthySensors / totalSensors) * 100).toFixed(1) : 0,
      },
      data: {
        totalDataPoints,
        recentDataPoints,
        dataIngestionRate: recentDataPoints / 24, // points per hour
        averageDataPointsPerSensor:
          totalSensors > 0 ? (totalDataPoints / totalSensors).toFixed(1) : 0,
      },
      health: {
        averageUptime: averageUptime.toFixed(1),
        averageErrorRate: averageErrorRate.toFixed(2),
        systemHealth:
          averageUptime > 95 && averageErrorRate < 5
            ? 'excellent'
            : averageUptime > 85 && averageErrorRate < 10
              ? 'good'
              : 'needs_attention',
      },
      distribution: {
...
async function getDeviceAnalytics(deviceId?: string) {
  ...
  const overallHealthScore =
    sensorHealthScore * 0.5 + connectivityScore * 0.3 + activityScore * 0.2;
...
async function getSensorAnalytics(sensorId?: string) {
  if (!sensorId) {
    return NextResponse.json({ success: false, error: 'Sensor ID is required' }, { status: 400 });
  }
  ...
  if (dataPoints.length > 0) {
    ...
    const avgAccuracy =
      dataPoints.reduce((sum, d) => sum + d.quality.accuracy, 0) / dataPoints.length;
    const avgConfidence =
      dataPoints.reduce((sum, d) => sum + d.quality.confidence, 0) / dataPoints.length;
...
async function getPerformanceAnalytics(timeRange: string) {
  ...
  const performanceMetrics = sensors.map(sensor => {
    const recentData = sensor.data.filter(d => new Date(d.timestamp).getTime() > cutoffTime);
    ...
    return {
      ...
      averageAccuracy:
        recentData.length > 0
          ? recentData.reduce((sum, d) => sum + d.quality.accuracy, 0) / recentData.length
          : 0,
  ...
  const averageAccuracy =
    performanceMetrics.reduce((sum, m) => sum + m.averageAccuracy, 0) / performanceMetrics.length ||
    0;
  const averageUptime =
    performanceMetrics.reduce((sum, m) => sum + m.uptime, 0) / performanceMetrics.length || 0;
  const averageErrorRate =
    performanceMetrics.reduce((sum, m) => sum + m.errorRate, 0) / performanceMetrics.length || 0;
...
async function getPredictiveAnalytics() {
  ...
  sensors.forEach(sensor => {
    ...
    const daysToExpiry = Math.ceil(
      (calibrationExpiry.getTime() - Date.now()) / (1000 * 60 * 60 * 24)
    );
...
function calculateDataFrequency(dataPoints: any[]): string {
  ...
  for (let i = 1; i < sortedData.length; i++) {
    const interval =
      new Date(sortedData[i].timestamp).getTime() - new Date(sortedData[i - 1].timestamp).getTime();
    intervals.push(interval);
  }

  const averageInterval = intervals.reduce((sum, interval) => sum + interval, 0) / intervals.length;

  if (averageInterval < 60000) {
    // Less than 1 minute
    return 'high_frequency';
  } else if (averageInterval < 300000) {
    // Less than 5 minutes
    return 'medium_frequency';
  } else if (averageInterval < 3600000) {
    // Less than 1 hour
...
function generateRecommendations(predictions: any[]): string[] {
  ...
  if (maintenancePredictions.length > 0) {
    recommendations.push(
      `Schedule maintenance for ${maintenancePredictions.length} devices showing health issues`
    );
  }
  ...
  if (calibrationPredictions.length > 0) {
    recommendations.push(
      `Plan calibration for ${calibrationPredictions.length} sensors expiring soon`
    );
  }
  ...
  if (qualityPredictions.length > 0) {
    recommendations.push(
      `Investigate data quality issues for ${qualityPredictions.length} sensors`
    );
  }
