...
interface MultiDimensionalTestResponse {
  ...
  executionResults: {
    ...
    byDimension: {
      unit: { passed: number; failed: number; coverage: number; duration: number };
      integration: { passed: number; failed: number; coverage: number; duration: number };
      e2e: { passed: number; failed: number; coverage: number; duration: number };
      security: { passed: number; failed: number; coverage: number; duration: number };
      performance: { passed: number; failed: number; coverage: number; duration: number };
    };
  };
  insights: {
    coverageGaps: string[];
...
`;
    ...
    const completion = await zai.chat.completions.create({
      messages: [
        {
          role: 'system',
          content: 'You are the OptiTest AI Controller, an expert in multi-dimensional testing orchestration, test automation, and quality assurance with deep knowledge of various testing methodologies and frameworks.'
        },
        {
          role: 'user',
    ...
    try {
      ...
      aiResponse = {
        ...
        insights: {
          coverageGaps: ['E2E testing needs improvement', 'Integration tests require more edge cases'],
        ...
        recommendations: {
          ...
          longTerm: ['Implement comprehensive test automation', 'Establish continuous testing pipeline']
        }
      };
    }
    ...
    const response: MultiDimensionalTestResponse = {
      ...
      metrics: {
        totalTests: aiResponse.executionResults.overall.passed + 
                     aiResponse.executionResults.overall.failed + 
                     aiResponse.executionResults.overall.skipped,
        executionTime,
        efficiency: Math.round((aiResponse.executionResults.overall.passed / 
          (aiResponse.executionResults.overall.passed + aiResponse.executionResults.overall.failed)) * 100),
