...
function generateMockTestData(): MonitorResponse {
  ...
  return {
    ...
    trends: {
      executionRate: Array.from(
        { length: 24 },
        () => Math.floor(Math.random() * 100) + 50,
      ),
      failureRate: Array.from(
        { length: 24 },
        () => Math.floor(Math.random() * 10) + 1,
      ),
      coverageTrend: Array.from(
        { length: 24 },
        () => Math.floor(Math.random() * 15) + 85,
      ),
      performanceTrend: Array.from(
        { length: 24 },
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      version: "1.0.0",
      endpoints: {
        "POST /api/testing/monitor":
          "Get real-time monitoring data with filters",
        "GET /api/testing/monitor": "Get current monitoring status",
        "GET /api/testing/monitor/stream":
