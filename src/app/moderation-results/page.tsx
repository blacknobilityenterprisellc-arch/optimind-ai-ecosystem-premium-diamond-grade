...
import {
  ...
  FilterState,
} from "@/components/ModerationFilterPanel";
...
const mockResults: ModerationResult[] = [
  {
    ...
    reasons: [
      ...
      "All models agree on safe classification",
    ],
  },
  {
    id: "2",
    ...
    processingTime: 4.1,
    reasons: [
      "Adult content detected by multiple models",
...
export default function ModerationResultsPage() {
  ...
  const [selectedResult, setSelectedResult] = useState<ModerationResult | null>(
    null,
  );
  ...
  const handleFilterChange = (filters: FilterState) => {
    ...
    if (filters.dateRange.start) {
      ...
      filtered = filtered.filter(
        (result) => new Date(result.createdAt) >= startDate,
      );
    }
    if (filters.dateRange.end) {
      ...
      filtered = filtered.filter(
        (result) => new Date(result.createdAt) <= endDate,
      );
    }
    ...
    if (filters.actions.length > 0) {
      filtered = filtered.filter((result) =>
        filters.actions.includes(result.recommendedAction),
      );
    }
      ...
      {selectedResult && (
        <Card className="mt-6">
          <CardHeader>
            ...
            <CardDescription>
              Detailed information for {selectedResult.imageId}
            </CardDescription>
          </CardHeader>
          <CardContent>
              ...
              <div>
                ...
                <div className="space-y-2 text-sm">
                  <div>
                    ...
                    {selectedResult.modelsUsed.join(", ")}
                  </div>
                  <div>
